...
AgoraRTC.getCameras().then((tracks)=>{
  if(tracks){
    if(tracks.length>1){
      $('#switch-channel').show();
    }else{
    ...
    const steam_source =$('#stream-source')
    tracks.forEach((track,index)=>{
      steam_source.append("<option value="+track.deviceId+ " "+(index==0 ? "selected='selected'":"")+">"+track.label+"</option>")
    })
  }
...
$('#stream-source').on('change',async(val)=>{
  // await stopCurrentChannel();
  // leave();
  const selectedmedia =$('#stream-source').val();

  await stopCurrentChannel().then(async()=>{
  [options.uid, localTracks.videoTrack] = await Promise.all([
    // Join the channel.
    client.join(options.appid, options.channel, options.token || null, options.uid || null),
    AgoraRTC.createCameraVideoTrack({
      cameraId:selectedmedia
    })]);
...
$('#local-player').on('click',function(e){
  if($('#local-player').hasClass('pinned')){
    $('#local-player').removeClass('pinned');
    $('#remote-playerlist').addClass('pinned');
  }
})
$('#remote-playerlist').on('click',function(e){
  if($('#remote-playerlist').hasClass('pinned')){
    $('#remote-playerlist').removeClass('pinned');
